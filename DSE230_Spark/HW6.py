...
#%%time
# Your code here
#f = open("../../Data/hw6-files-10mb.txt","r")
#f = open("../../Data/hw6-files-4gb.txt","r")
...
def safe_parse(raw_json):
    ...
    if 'user' in json_object:
        user_id = json_object['user']['id_str'].strip().encode('utf-8')
        text = json_object['text'].strip().encode('utf-8')
    else:
        user_id = None
        text = None
...
# your code here
parsedRDD = RDD.map(lambda x: safe_parse(x)).filter(lambda (u,t): u<>None).cache()
...
#%%time
# your code here
print_users_count(parsedRDD.keys().distinct().count())
...
# your code here
import pickle
partitions = pickle.load(open('../../Data/users-partition.pickle'))
...
#%%time
# your code here
def find_partition(user_id):
    try:
        partition = partitions[user_id]
    except:
        partition = 7
    return partition

# create user/partition/tweet (upt) RDD for future use
uptRDD = parsedRDD.map(lambda(u,t):((u,find_partition(u)),t)).cache()

# create pairsRDD from uptRDD
pairsRDD = uptRDD.map(lambda ((u,p),t):(p,1)).reduceByKey(lambda x,y:x+y).sortByKey()
...
#%%time
tRDD = uptRDD.flatMapValues(lambda t: tok.tokenize(t)).map(lambda ((u,p),t):(u,t,p,1)).distinct().cache()

print_count(tRDD.map(lambda (u,t,p,c):t).distinct())
...
#%%time
topToksRDD = tRDD.map(lambda (u,t,p,c):(t,c)).reduceByKey(lambda x,y:x+y).filter(lambda (t,c):c>=100).cache()

print_count(topToksRDD)
print_tokens(topToksRDD.takeOrdered(20, lambda (t,c):-c))
...
#%%time
relpopRDD = tRDD.map(lambda (u,t,p,c):((t,p),c)).reduceByKey(lambda x,y:x+y).map(lambda ((t,p),c_k):(t,(p,c_k))).join(topToksRDD).map(lambda (t,((p,c_k),c_all)): (t,p,get_rel_popularity(c_k,c_all)))

#for i in range(8):
#    toks = relpopRDD.filter(lambda (t,p,pop): p==i).map(lambda (t,p,pop):((-pop,t),p)).sortByKey().map(lambda((pop,t),p):(t,-pop)).take(10)
#    print_tokens(toks,float(i))
    
for i in range(8):
    toks = relpopRDD.filter(lambda (t,p,pop): p==i).map(lambda (t,p,pop):(t,pop)).takeOrdered(10, lambda (t,pop):(-pop,t))
    print_tokens(toks,float(i))
